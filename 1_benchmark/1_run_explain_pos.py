...
parser.add_argument("--pos_upper", dest="pos_upper", default=None, type=int)
parser.add_argument("--pos_lower", dest="pos_lower", default=None, type=int)
...
pos_upper = args.pos_upper
pos_lower = args.pos_lower
...
params['leftpos'] = 10000 - pos_upper
params['rightpos'] = 10000 + pos_lower
...
test_loader = DataLoader(list(zip(x_test, y_test_onehot)), batch_size=200, 
...
input_tensor = torch.from_numpy(x_test[np.random.randint(0, x_test.shape[0], 128),:,:]).to(device)
# saliancy score
saliency_length = input_saliancy_location(model, input_tensor, n_class=100, use_abs=True)
